...
module.exports = function (app) {
  ...
  .post(function (req, res){
    ...
    if (!req.body.issue_title || !req.body.issue_text || !req.body.created_by) {
      res.json({result: 'required not completed'});
    } else {
      var new_entry = {};
  ...
  .put(function (req, res){
    ...
    if (Object.keys(updated_entry).length === 0) {
      ...
      MongoClient.connect(CONNECTION_STRING, function(err, db) {
        ...
        .findAndModify({_id: ObjectId(id)}, {$set: updated_entry}, {new: true}, (err,doc) => {
          if (err) {
            console.error(err);
  ...
  .delete(function (req, res){
    ...
    var id = req.body._id;
    if (id === '') {
      res.json({result: 'no _id sent - _id error'});
      return;
    }

    MongoClient.connect(CONNECTION_STRING, function(err, db) {
      db.collection(project)
      .deleteOne({_id: id}, (err, doc) => {
        if (err) {
          console.error(err);
